...
if ( ! class_exists( 'Exopite_Simple_Options_Framework' ) ) :
	...
	class Exopite_Simple_Options_Framework {
		...
		public function load_textdomain() {
			...
			load_textdomain( 'changenowio', $mofile );
		}
			...
			if ( ! empty( $required_key_array ) && ! empty( $this->config ) ) {
				foreach ( $required_key_array as $key ) :
					if ( ! isset( $this->config[ $key ] ) ) {
						...
						$this->errors->add( "missing_config_key_{$key}", sprintf( eac_attr__( '%s is missing in the configuration array', 'changenowio' ), $key ) );
					}
				endforeach;
		...
		public function display_admin_error() {
			...
			if ( ! empty( $errors_array ) ) {
				...
				$message .= eac_attr__( 'Irks! An un-known error has occurred.', 'changenowio' );
			}
		...
		public function display_options_page_header() {
			echo '<form method="post" action="options.php" enctype="multipart/form-data" name="' . $this->unique . '" class="exopite-sof-form-js ' . $this->unique . '-form" data-save="' . esc_attr__( 'Saving...', 'changenowio' ) . '" data-saved="' . esc_attr__( 'Saved Successfully', 'changenowio' ) . '">';
			...
			echo '<h1> <img style="vertical-align:bottom" src="//changenow.io/images/icons/mstile-144x144.png" height="24" /> ' . __( 'ChangeNow.io Plugin Settings', 'changenowio' ) . '</h1>';

			// echo '<span class="exopite-sof-search-wrapper"><input placeholder="settings search..." type="text" class="exopite-sof-search"></span>';

			echo '<fieldset><span class="exopite-sof-ajax-message"></span>';
			submit_button( esc_attr__( 'Save Settings', 'changenowio' ), 'primary ' . 'exopite-sof-submit-button-js', $this->unique . '-save', false, array() );
			echo '</fieldset>';
			echo '</header>';
		}
		...
		public function display_options_page_footer() {
			...
			echo '<fieldset><span class="exopite-sof-ajax-message"></span>';
			submit_button( esc_attr__( 'Save Settings', 'changenowio' ), 'primary ' . 'exopite-sof-submit-button-js', '', false, array() );
			echo '</fieldset>';
